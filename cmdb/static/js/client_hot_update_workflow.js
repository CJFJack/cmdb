...
function initModalSelect2(){
    ...
    $select2Area = $('#area').select2( {
        ajax: {
            url: '/myworkflows/list_game_project_area_detail/',
            dataType: 'json',
            type: 'POST',
            delay: 250,
            data: function (params) {
    ...
    $select2UpdateVersion.on("select2:select", function (e) { display_cdn_version("select2:select", e, $( this )); });
...
// 监听添加cdn目录事件
function listenMyAdd(){
    $("#myAdd").click( function(){
        var add_str = `<div class="form-group create_update_version">
                          <div class="col-sm-6">
                              <input type="text" class="form-control input_cdn_dir" placeholder="s1">
                          </div>
                          <button class="btn btn-danger btn-sm myRemove" type="button">x</button>
                        </div>
                        `
        $("#insert_cdn_dir_after").after(add_str);
...
// 如果更新的游戏项目是手游, 由用户自己填写版本号和cdn目录
function initInputUpdateVersion(){
    var add_str = '<div class="form-group create_update_version" id="create_update_version">' +
                    '<label class="col-sm-12">更新版本号</label>' +
                    '<div class="col-sm-6">' +
                        '<input class="form-control" id="update_version" placeholder="104400000"></input>' + 
                    '</div>' +
                    '<div class="alert alert-danger col-sm-4" id="show_update_version_msg" style="display: none;">' +
                        '请填写要更新的前端版本号!' +
                    '</div>' +
                  '</div>' +
                  '<div class="form-group create_update_version" id="insert_cdn_dir_after">' +
                    '<label class="col-sm-2" id="create_cdn_dir">cdn目录</label>' +
...
function reset_all(name, evt, el){
    if (name == "select2:select" || name == "select2:select2"){
        ...
        if ( $(el).attr('id') == 'project' ) {
            $("#area").val('0').trigger('change');
            // 重置备用主程
            $("#backup_dev").val('0').trigger('change');
            ...
            // 如果项目是页游，版本号和cdn目录需要选择
            // 如果项目是手游，版本号和cdn目录自己填写
            var inputs = {'project': $("#project").select2('data')[0].id};
            var encoded=$.toJSON( inputs );
            var pdata = encoded;
            $.ajax({
                type: "POST",
                url: "/myworkflows/get_client_type/",
                async: true,
                contentType: "application/json; charset=utf-8",
                data: pdata,
                success: function (data) {
                    ...
                    if ( data.client_type == 0 ){
                        $("#show_client_type").show();
                        initInputUpdateVersion();
                        game_type = 0;
                    } else {
                        $("#show_client_type").hide();
...
function get_cdn_and_version_by_game_type() {
    if ( game_type == 1 ) { return  table.rows('.selected').data();}
    ...
    $("#example-basic").steps({
        ...
        onStepChanging: function (event, currentIndex, newIndex) {
            if ( currentIndex<newIndex & currentIndex == 0){
                ...
                if ( project == '0' ){
                    $("#show_project_msg").show();
                    return false;
                } else {
                    $("#show_project_msg").hide();
                }
                ...
                if ( game_type == 1 ) {
                    ...
                    var input_cdn_dir_array = $(".input_cdn_dir").map( function(){ return $(this).val().trim(); } ).get();
