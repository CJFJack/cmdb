...
$(document).ready(function () {
    ...
    $("#bt-commit").confirm({
        ...
        confirm: function (button) {
            ...
            if ($("#bt-add").length > 0 && $('#edit_client').val() === 'True') {
                if (!checkUpdateContent()) {
                    return false;
                }
                if ($("#client_attention").val() === '') {
            ...
            var inputs = {
                ...
                'client_content': client_type_cdn_root_dir_version,
                'ask_reset': ask_reset,
    ...
    $("#bt-add").click(function (event) {
        ...
        $.ajax({
            type: "POST",
            url: "/myworkflows/check_webapi_client_type/",
            contentType: "application/json; charset=utf-8",
            async: false,
            data: pdata,
            success: function (data) {
                if (data.success) {
                    if (data.client_type) {
                        var add_str = '<div class="col-sm-12 form-group update_content">' +
                            ...
                            '</div>' +
                            '<div class="col-sm-2">' +
                            '<input type="text" class="form-control" style="width: 100%; display: none" placeholder="更新版本">' +
                            '</div>' +
...
function checkUpdateContent() {
    ...
    if ($(".update_content").length == 0) {
        ...
        $(".update_content").each(function (i, e) {
            ...
            if (client_type.parent().css('display') == 'block' && client_type.val() == '0') {
            ...
            var update_version_resource_name = update_version.val();

            if (update_version_resource_name == '' && $('#edit_client').val() === 'True') {
                show_update_content_msg.html('请填写版本号!');
                show_update_content_msg.show();
                result = false
                return false
            } else {
