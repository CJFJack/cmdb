...
function initClientType(selector) {
    ...
    $select2ClienType.on("select2:select", function (e) { reset_cdn("select2:select", e, $( this )); });
...
function initCDNRootUlr(selector){
    ...
    $select2CDNRootUll.on("select2:select", function (e) { reset_cdn("select2:select", e, $( this )); });
}
...
function checkUpdateContent(){
    ...
    if ( $(".update_content").length == 0 ) {
        ...
        $(".update_content").each( function(i, e) {
            ...
            if ( client_type.val().split('_')[1] == 'ios' ) {
            ...
            if ( client_type.val().split('_')[1] == 'android' ) {
            ...
            /* 判断cdn 目录的数据和update_version的格式
            cdn dir 的值 chuangyu_t2
            更新版本的值Achuangyu_03037
            */

            var cdn_dir_resource_name = cdn_dir.val().split('_')[0]
            var update_version_resource_name = update_version.val().split('_')[0].slice(1)
            console.log(cdn_dir_resource_name)
            console.log(update_version_resource_name)
...
$(document).ready(function() {
                ...
                client_type_cdn_root_dir_version.forEach( function(el, index) {
                    if ( index == 0 ) {
                        var add_str = '<div class="form-group client_type_cdn_root_dir_version">' +
                                        ...
                                        '</label>' +
                                        '<div class="col-sm-3">' +
                                            '<input class="form-control" readonly="readonly" value=' + el.client_type +'></input>' +
                                        '</div>' +
                                        '<div class="col-sm-3">' +
                                            '<input class="form-control" readonly="readonly" value=' + el.cdn_root_url +'></input>' +
                                        '</div>' +
                                        '<div class="col-sm-3">' +
                                            '<input class="form-control" readonly="readonly" value=' + el.cdn_dir +'></input>' +
                                        '</div>' +
                                        '<div class="col-sm-3">' +
                                            '<input class="form-control" readonly="readonly" value=' + el.version +'></input>' +
                                        '</div>' +
                                      '</div>'
                    } else {
    ...
    $("#bt-add").click(function(event) {
        ...
        var myRemove_bt = $($("#create_update_version").next().children().get(4))
