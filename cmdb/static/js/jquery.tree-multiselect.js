...
exports.createItem = function (id, value, text, description, initialIndex, section, ip, srv_id, server_id, host_id, platform_id, platform, gtype, linkurl, gameserverid) {
  return {
    type: 'item',
    id: id,
    value: value,
...
Tree.prototype.createAst = function (options) {
  ...
  options.each(function () {
    ...
    var optionIp = option.getAttribute('data-ip');
    var optionSrvId = option.getAttribute('data-srv');
    var optionGTYPE = option.getAttribute('data-gtype');
    var optionServerId = option.getAttribute('data-server');
    var optionHostId = option.getAttribute('data-host');
    var optionPlatForm = option.getAttribute('data-pf');
    var optionPlatFormName = option.getAttribute('data-platform');
    var optionLinkUrl = option.getAttribute('data-linkurl');
    var optionGameServerId = option.getAttribute('data-gameserverid');
    var optionObj = Ast.createItem(id, optionValue, optionName, optionDescription, optionIndex, section, optionIp, optionSrvId, optionServerId, optionHostId, optionPlatForm, optionPlatFormName, optionGTYPE, optionLinkUrl, optionGameServerId);
...
exports.createNode = function (tag, props) {
  ...
  if (props) {
    ...
    if (props.linkurl) {
      $(node).html('')
      var link_to_group = "<a href=/users/group_section/?group_id=" + props.linkurl + ">" + props.text + "</a>"
      $(node).append(link_to_group)
    }
  }
  return node;
};
...
exports.createSelection = function (option, treeId, createCheckboxes, disableCheckboxes) {
  ...
  if (option.ip) {
    props['data-ip'] = option.ip;
  }
  if (option.srv_id) {
    props['data-srv'] = option.srv_id;
  }
  if (option.gtype) {
    props['data-gtype'] = option.gtype;
  }
  if (option.server_id) {
    props['data-server'] = option.server_id;
  }
  if (option.host_id) {
    props['data-host'] = option.host_id;
  }
  if (option.platform_id) {
    props['data-pf'] = option.platform_id;
  }
  if (option.platform) {
    props['data-platform'] = option.platform;
  }
  if (option.linkurl) {
    props['data-linkurl'] = option.linkurl;
  }
  if (option.gameserverid) {
    props['data-gameserverid'] = option.gameserverid;
  }
