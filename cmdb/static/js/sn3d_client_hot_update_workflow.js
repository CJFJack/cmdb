...
function initSelect2UpdateVersion() {
    ...
    var add_str = '<div class="form-group create_update_version" id="create_update_version">' +
        '<label class="col-sm-12" style="display: none">更新版本号</label>' +
        '<div class="col-sm-6" style="display: none">' +
        '<select id="update_version" style="width: 100%">' +
        '<option value="0" selected="selected">选择更新版本号</option>' +
        '</select>' +
        '</div>' +
        '<div class="alert alert-danger col-sm-4" id="show_update_version_msg">' +
        '请选择要更新的前端版本号!' +
        '</div>' +
        '</div>'
...
function initSelect2CdnRootUrl() {
    // 初始化cdnurl
    $select2cdnrooturl = $("#select_cdn_root_url").select2({
        ajax: {
    ...
    $select2cdnrooturl.on("select2:select")

    // 初始化cdnarea
    $select2cdnarea = $("#input_cdn_dir").select2({
        ajax: {
            ...
            data: function (params) {
                return {
                    ...
                    project: project,
                    area_name: $('#area').val(),
                    cdn_root_url: $('#select_cdn_root_url').select2('data')[0].id,
                    client_type: '',
                };
            },
    ...
    $select2cdnarea.on("select2:select")
}
                    ...
                    "columnDefs": [
                        {
                            ...
                            'render': function (data, type, full, meta) {
                                return '<input name="radio_cdn_root_url" checked type="radio">';
                            },
                        },
                        {
                            'targets': [1, 2, 6],
                        ...
                        },
                        {
                            ...
                            'orderable': false,
                            'render': function (data, type, full, meta) {
                                return '<select style="width: 100%" class="form-control" id="select_cdn_root_url">' +
                                    '<option value="" disabled selected style="display:none;">点击下拉框选择cdn域名</option>' +
                                    '</select>';
                            },
                        },
                        {
                            ...
                            'orderable': false,
                            'render': function (data, type, full, meta) {
                                return '<select style="width: 100%" class="form-control" id="input_cdn_dir">' +
                                    '<option value="" disabled selected style="display:none;">点击下拉框选择cdn目录</option>' +
                                    '</select>';
                                // return '<input type="text" class="form-control" id="input_cdn_dir" placeholder="t1">';
                            },
                        },
                        {
                            'targets': 5,
                            ...
                            'orderable': false,
                            'render': function (data, type, full, meta) {
                                return '<input type="text" class="form-control" id="input_update_version" placeholder="res/last5sn3d">';
                            },
                        },
                    ],
                });

                initSelect2CdnRootUrl();

                $('#mytable tbody').on('click', 'input[type="radio"]', function (e) {
...
function reset_all(name, evt, el) {
    if (name == "select2:select" || name == "select2:select2") {
        ...
        if ($(el).attr('id') == 'area') {
            ...
            display_cdn_version("select2:select", '', '');
...
// 获取区服列表数据
function get_game_server_list_data(cdn_area_dir, cdn_root_url) {
    var result = true;
    var inputIds = {
        'cdn_area_dir': cdn_area_dir,
        'cdn_root_url': cdn_root_url,
        'project': project,
    };
    ...
    $.ajax({
        type: "POST",
        url: "/myworkflows/get_game_server_list/",
        contentType: "application/json; charset=utf-8",
        async: false,
        data: pdata,
        success: function (data) {
    ...
    $("#example-basic").steps({
        ...
        onStepChanging: function (event, currentIndex, newIndex) {
            if (currentIndex < newIndex & currentIndex == 0) {
                ...
                if (game_type == 1) {
                    var client_version = $("#input_update_version").val();
                    var input_cdn_dir = $('#input_cdn_dir').select2('data')[0].text;
                    var input_update_version = $('#input_update_version').val();
                    var select_cdn_root_url = $('#select_cdn_root_url').select2('data')[0].id;
                    //console.log(select_cdn_root_url);
                    if (select_cdn_root_url == '') {
                        $("#show_update_version_msg").show();
                        $("#show_update_version_msg").html('请选择cdn根目录!');
                        return false;
                    } else {
                        $("#show_update_version_msg").hide();
                    }
                    if (input_cdn_dir == '') {
                        $("#show_update_version_msg").show();
                        $("#show_update_version_msg").html('请选择cdn目录!');
                        return false;
                    } else {
                        $("#show_update_version_msg").hide();
                    }
                    if (input_update_version == '') {
                        $("#show_update_version_msg").show();
                        $("#show_update_version_msg").html('请填写更新版本号!');
                        return false;
                    } else {
                        $("#show_update_version_msg").hide();
                    }

                    var selected_cdn_client_version = table.row('.odd').data();
                    ...
                    var input_cdn_dir = $('#input_cdn_dir').select2('data')[0].id;
                    // 如果没有添加任何的cdn目录
                    if (input_cdn_dir == '') {
                        $("#show_update_version_msg").show();
        ...
        onStepChanged: function (event, currentIndex, priorIndex) {
            ...
            if (currentIndex == 3) {
                var area_name = $("#area").val();
                var version = $("#input_update_version").val();
            ...
            if (currentIndex == 5) {
                ...
                var cdn_root_url = $('#select_cdn_root_url').select2('data')[0].id;
                var cdn_dir = $('#input_cdn_dir').select2('data')[0].text;
                var client_version = $('#input_update_version').val();
                var add_str = '<div class="form-group cdn_version">' +
                    ...
                    '</div>' +
                    '<div class="col-sm-3">' +
                    '<input class="form-control" readonly="readonly" value=' + cdn_dir + '></input>' +
                    '</div>' +
                    '<div class="col-sm-3">' +
        ...
        onFinished: function (event, currentIndex) {
            ...
            var pair_code = get_select_or_gen_pair_code();
            var order = $("#order").val();
            var cdn_area_dir = $("#input_cdn_dir").select2('data')[0].text;
            var cdn_root_url = $('#select_cdn_root_url').select2('data')[0].text;

            // 文件列表数据
            var cdn_client_version_list = get_game_server_list_data(cdn_area_dir, cdn_root_url);
