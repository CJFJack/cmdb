...
def cmdb_callback(msg, success, update_type, content_object_id, uuid):
    ...
    try:
        log = RsyncLog()
        url = 'https://192.168.100.66/api/RsyncOnFinishedCallBack/'
        headers = {
        ...
        requests.post(url, headers=headers, timeout=15, json=payload, verify=False)
        log.logger.info('回调cmdb接口成功-%s' % (uuid))
    except Exception as e:
